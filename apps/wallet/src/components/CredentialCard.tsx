...
export function CredentialCard({
  ...
  return (
    // The card itself is the flex column — never give it an inner
    // percentage-height wrapper. WebKit resolves a child's h-full against
    // an aspect-ratio box's BORDER box, so the column overflows by the
    // padding and overflow-hidden eats the last row (Safari-only clipping).
    <div
      className={`guilloche ${
        theme.pattern === "tide" ? "guilloche-tide" : ""
      } flex aspect-[8/5] w-full flex-col overflow-hidden rounded-3xl border border-[rgb(201_168_106/0.35)] p-6 text-[#ece7d3] shadow-[0_18px_44px_rgb(0_0_0/0.45)]`}
      style={
        {
    ...
    >
      ...
      <div className="mt-auto flex items-end justify-between gap-4">
        <div className="min-w-0">
          ...
          {face != null && face.fields.length > 0 && (
        ...
        <div className="flex shrink-0 flex-col items-end gap-1 text-right">
          ...
          {face != null && (
            <p className="font-mono text-[9px] tracking-[0.18em] text-[#c9a86a]/75">
