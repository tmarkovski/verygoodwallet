 ...
 * dist/vgw_shop/wrangler.json — the exact artifact `wrangler deploy` uploads —
 * so this catches script-startup failures that only exist in the Cloudflare
 * runtime (module-scope `new URL(..., import.meta.url)` in a bundled dep,
 * non-handler named exports, a broken Durable Object migration). Beyond
 * booting, it exercises the session lifecycle: create a verification
 * session, then poll its status — which round-trips through the
 * VerificationSessions Durable Object under real workerd.
 *
 * Usage (after `pnpm --filter @vgw/shop build`):
 *   pnpm --filter @vgw/shop smoke
 */
...
if (!existsSync(path.join(appDir, "dist/vgw_shop/wrangler.json"))) {
  console.error("smoke: dist/vgw_shop/wrangler.json missing — run `pnpm --filter @vgw/shop build` first");
  process.exit(1);
}
...
const smokeConfig = "dist/vgw_shop/wrangler.smoke.json";
const { routes: _routes, ...config } = JSON.parse(
  readFileSync(path.join(appDir, "dist/vgw_shop/wrangler.json"), "utf8"),
);
