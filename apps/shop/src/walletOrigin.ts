 ...
 * no production default, because until the M6 cutover the apex domain
 * serves the legacy app, which has no /present route — a guessed origin
 * would dead-end the verification. Production builds must set
 * VITE_WALLET_ORIGIN (the Deploy Shop workflow requires it); without it the
 * UI shows a configuration notice instead of a broken link/QR.
 */
...
export function clientWalletOrigin(): string | null {
  ...
  if (configured !== undefined && configured !== "") {
    ...
    console.warn(
      `vgw-shop: VITE_WALLET_ORIGIN is not an absolute origin, ignoring it: ${configured}`,
    );
  }
