/**
 * The shop's published proof alphabet (MIGRATION §8, Appendix D.3): ONE
 * range-params document per verifier, served at `/.well-known/credkit-params`
 * and consumed verbatim by every prover.
 *
...
import { resolveParamsSeed, type ShopBindings } from "./env.js";

/** Domain-separation tag for THIS verifier's alphabet (rentals uses its own). */
const PARAMS_DST = "VGW-SHOP-CREDKIT-RANGE-PARAMS-V1";
...
export interface ShopRangeParams {
...
const cache = new Map<string, Promise<ShopRangeParams>>();

async function mint(seed: string): Promise<ShopRangeParams> {
...
export function getRangeParams(env: ShopBindings): Promise<ShopRangeParams> {
