 ...
 *
 * ```
 * POST /api/verification        rentals UI starts a driver-verification session
...
export function createApp(): Hono<{ Bindings: RentalsBindings }> {
  ...
  // The wallet calls /oid4vp/response cross-origin; /api/* is for the
  // rentals UI but reflecting there too is harmless (no cookies, no ambient
  // auth).
  ...
  app.post("/oid4vp/response", async (c) => {
    ...
    const presentations =
      typeof vpToken === "object" && vpToken !== null && !Array.isArray(vpToken)
        ? (vpToken as Record<string, unknown>)[RENTAL_QUERY_ID]
        : undefined;
    ...
    if (typeof presentation !== "object" || presentation === null) {
      return c.json(
        ...oauthError(
          400,
          "invalid_request",
          `vp_token carries no presentation for credential query "${RENTAL_QUERY_ID}"`,
        ),
      );
    }
    ...
    // A zkAgeProof on the presentation selects the tier-2 age route. The
    ...
    const outcome: SessionOutcome = result.verified
      ? {
          ...evaluateRentalPolicy(
            result.credentials.map((r) => r.credential),
            zkAgeProof,
          ),
          vpToken,
