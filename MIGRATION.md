...
| **N5** | Two commits (D.5 staging). **N5a ✅ landed — foundation + issuance:** bundled `https://verygoodwallet.com/contexts/utopia-resident/v1` (credential/subject types, `districtName`, `stateFips` + `postalCode` coerced `xsd:unsignedInt`); `buildUtopiaResidentRegistration` + `UTOPIA_RESIDENT_NUMERIC_DECLARATIONS` (`uint64` twins at `/credentialSubject/stateFips` and `/credentialSubject/postalCode`; values serialized as canonical decimal STRINGS whose context typing is pinned by an issuance-fails-closed regression — see the D.5 as-executed note); the shared Utopia geography (six districts, coastal 11/12/13 vs inland 21/22/23, disjoint 5-digit postal blocks; crypto-free `@vgw/vc-kit/geography` subpath so the DMV UI's dropdown pulls no credkit); vc-kit set-params mirror (`mintSeededSetParams`/codec/hash/`verifySetParams`) + `membershipClaims` pass-through on `createCredkitPresentation`; protocols params document gains fail-closed `sets` validation (D.5.5 shape). The DMV issues `utopia_resident_registration` as a second configuration — offer-body discriminator (`credential_configuration_id`, absent = DL for API compat), district/postal validated against the geography, same blind-issuance ritual and issuer DID — **plus a hardening the two-config world forced: the signed pre-auth code and access token now bind the offered configuration id** (pre-N5 they bound nothing — one config made the constant check sufficient; now the credential endpoint refuses a token↔request mismatch and the legacy unbound shape, both pinned by vectors). Wallet: offer→issue path needed ZERO changes (config-agnostic — proven by a resident-offer flow test); rendering gained the resident kind label + flat-subject claims view. Facade-level showcase-B tests land here (membership happy/wrong-set/non-member-throw; two-sided postalCode range), wired to live verifiers at N5b. Counts: 393+12 → **432 passed + 12 e2e-gated** (vc-kit 46→66, protocols 104→106, dmv 41→56, wallet 81→83). **N5b ✅ landed — showcases B + C live.** Wallet (D.5.7): the three N3 rejections came out — `matchCredentials` matches per query in query order (`MatchedRequest`; a composite = multi-query or equalities, all-or-nothing: an unsatisfiable query fails loudly naming the query + the DMV re-issue fix, while single-query zero-candidates stays a renderable result), `predicateOption` treats `membership` like `range` (declared-twin gate, consent line "is one of the verifier's published set — the value itself stays hidden"; the set's SIZE surfaces after the fetch, in the inspector — consent stays network-free), the D.3 pinning ritual extends to `sets[set_id]` (ONE fetch per params_uri serves range + sets; double-hash pin, `verifySetParams` once, cached by hash), and `vgw_equalities` maps query-id refs → statement indices by query order — `link_secret` refs only, a `path` ref rejected loudly. New `presentComposite` ceremony (fixed shape, no tier ladder; ONE link secret derived once across statements, each with its own blind; same zeroing + abort discipline) posts ONE graph VP under the FIRST query's id (D.5.1); `Present.tsx` gained the composite consent (per-statement prove/disclose sections + the one-holder linkage line). Rentals: params doc publishes `sets.coastal` (fips 11/12/13 in publication order, DST `VGW-RENTALS-CREDKIT-SET-PARAMS-coastal-V1`); `POST /api/verification` gained the `{flow}` discriminator (absent body = standard; unknown → 400); `buildResidentRateDcqlQuery` offers the D.5.3 composite (two predicate-only queries, empty claim_sets, NO disclosure alternatives, one link-secret equality); the state token's memory generalized per D.5.6 (`{flow, offer}` with statement-indexed `range[]`/`membership[]`/`equalities` — VERSIONED, not back-compatible: a pre-N5b `predicates` token fails the shape gate closed, same semantics as a seed rotation); the response endpoint reads the VP under the flow's first query id, route-peeks ALL THREE envelope counts against the offer, restates range + membership (isolate params) + equalities + `[dmv, dmv]`, and `evaluateResidentRatePolicy` returns `allowed` with `disclosed = {}` + a `composite` exhibit (cutoff, set members, equality) the UI narrates. `RentalGate` gained the second entry point ("Check coastal resident rate" — eligibility only; the lot's gate still runs the standard check) with the empty disclosed set as the headline. Counts: 432+12 → **452 passed + 14 e2e-gated** (wallet 83→91, rentals unit 18→30); e2e: rentals 24→38 under `VGW_E2E=1` (gated 6→8: composite happy path + inland prover-throw against BUILT Workers, the DMV blind-issuing BOTH credentials to one link secret over HTTP), shop 33/33, dmv 41/41 |
...
**As executed at N5b** (no design deviations; five details the code pass nailed down):

- **The rentals token memory is VERSIONED, not migrated (D.5.6).** The state token's
  `predicates: OfferedRangeClaim[]` field was *replaced* by `{flow: "standard"|"resident-rate",
  offer: {statements, range[], membership[], equalities}}` (statement-indexed throughout;
  equalities as link-secret groups over statement indices — the only kind offered, so no
  per-ref kind tag until pointer-twin equality actually ships). The response endpoint runs a
  fail-closed shape gate over the decoded payload: a pre-N5b token (or any unknown shape) is
  "state is invalid or expired", pinned by a vector. In-flight sessions across the N5b deploy
  die — deliberately the same semantics as a params-seed rotation, and the 600 s TTL bounds
  the blast radius.
- **Reading the D.5.1 vp_token needed one nuance: the key comes from the FLOW, not the wire.**
  The verifier cannot ask the (untrusted) vp_token which query it answers; the signed token's
  `flow` selects the expected key — the standard query id, or the composite's first query id
  (`utopia_dl_over25`) — and anything else is "carries no presentation for credential query X".
- **Composite = multi-query OR equalities; single-query behavior is bit-preserved.** The
  matcher throws the loud unsatisfiable error (naming the query + the DMV re-issue fix) only
  for composites — a single-query request with zero candidates stays a *result* the consent
  page renders as guidance, exactly the pre-N5b UX. Within a composite, a query WITHOUT
  `vgw_predicates` is a plain disclosure statement (the DCQL match's pointers); with them, the
  shape is fixed: prove the claims, disclose exactly the `claim_set`.
- **The membership consent line carries the set id, not the set size.** Consent renders before
  any network I/O (the N3 invariant), and the wire claim pins only `set_id` + `params_hash` —
  the member count is knowable only after the params fetch, so it surfaces in the pinning
  inspector event (and the verifier-side exhibit lists the full public member list). The D.5.7
  "+ set size" consent nicety would have cost a pre-consent fetch; declined.
- **The prover-refusal copy generalized.** The wallet's §9 fail-closed error now matches both
  refusal shapes (`does not fit` ranges, `not a member of the set` memberships) and says
  "does not satisfy the verifier's requirement" — the range-specific "bound" wording died with
  its single-claim assumption.
